class BusinessProfilesController < ApplicationController
  ...
  def create
    create = AddBusinessProfile.new(current_user, @form).tap do |action|
      action.on(:business_profile_added_successfully) do |business_profile|
        flash[:notice] = "Successfully added the business profile."
        redirect_to edit_business_profile_path(business_profile)
      end
      action.on(:business_profile_adding_failed) {render :new}
    end
    create.with(params[:business_profile])
  end
